...
export default function ChildGuidanceDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  async function loadPlan() {
    const res = await fetch(`/api/admin/child-guidance-plans/${id}`);
  ...
  async function updateDay() {
    if (!day) return;
    await fetch(`/api/admin/child-guidance-plans/${id}/weeks/${week!.id}/days/${day.id}`, {
      method: "PUT",
  ...
  async function updatePlanMedia(imageUrl: string, videoUrl: string) {
    await fetch(`/api/admin/child-guidance-plans/${id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  async function updateWeek() {
    if (!week) return;
    await fetch(`/api/admin/child-guidance-plans/${id}/weeks/${week.id}`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        title: week.title,
  ...
  async function addContent(e: React.FormEvent) {
    ...
    const endpoint = plan?.isDayWise && day
      ? `/api/admin/child-guidance-plans/${id}/weeks/${week.id}/days/${day.id}`
      : `/api/admin/child-guidance-plans/${id}/weeks/${week.id}`;
  ...
  async function deleteContent(contentId: string) {
    ...
    const endpoint = plan?.isDayWise
      ? `/api/admin/child-guidance-day-content/${contentId}`
      : `/api/admin/child-guidance-content/${contentId}`;
  ...
  async function addWeek() {
    ...
    setAddingWeek(true);
    const res = await fetch(`/api/admin/child-guidance-plans/${id}/weeks`, { method: "POST" });
  ...
  return (
    <AdminLayout>
      <Link href={`${capabilities.basePath}/child-guidance`} className="flex items-center gap-1 text-sm text-[var(--primary)] hover:underline">
