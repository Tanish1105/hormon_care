...
export default function PlansPage() {
  ...
  async function loadPlans() {
    ...
    if (!res.ok) {
      setError("Could not load plans. Please restart the server.");
      setPlans([]);
      return;
    }
  ...
  async function createPlan(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      setError(data.error || "Could not create plan. Please restart the server (npm run dev).");
      setLoading(false);
      return;
    }
    setTitle("");
  ...
  return (
    <AdminLayout>
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Personalized Plan</h1>
          <p className="text-sm text-slate-500 sm:text-base">Manage week-wise Personalized Plan</p>
        </div>
      ...
      {showForm && (
        <Card className="mt-6">
          <h2 className="mb-4 font-semibold text-slate-900">Create New Plan</h2>
          <form onSubmit={createPlan} className="space-y-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <FileUpload label="Plan Image Upload" accept="image" value={imageUrl} onChange={setImageUrl} />
              <FileUpload label="Plan Video Upload" accept="video" value={videoUrl} onChange={setVideoUrl} />
            </div>
            <p className="text-xs text-slate-500">
              {isDayWise
                ? "This creates Week 1–N, each with Day 1–7. Add daily content afterwards."
                : "This creates Week 1, Week 2, ... Add content to each week afterwards."}
            </p>
      ...
      <div className="mt-6 space-y-4">
        ...
        {plans.length === 0 && (
          <p className="text-center text-slate-500 py-8">No plans yet. Create a New Plan.</p>
        )}
      </div>
    </AdminLayout>
  );
