...
import { Dumbbell, Video, ExternalLink, ImageIcon, Baby } from "lucide-react";
    ...
    <div className="rounded-xl border bg-slate-50 p-4">
      <div className="flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {item.type === "YOUTUBE" && (
            <FullscreenYoutube contentId={item.id} source="garbha" secure title={item.title} />
          )}
...
export default function PatientGarbhaSanskarPage() {
  const [data, setData] = useState<GarbhaData | null>(null);
  ...
  const loadDashboard = useCallback(() => {
    fetch("/api/patient/dashboard")
      ...
      .then((d) => {
        setData(d);
        setSelectedWeek(d.garbhaUnlockedWeek || 1);
        setSelectedDay(d.garbhaUnlockedDay || 1);
      });
  }, []);
  ...
  const plan = data.profile.garbhaPlan;
  const { garbhaUnlockedWeek, garbhaUnlockedDay } = data;
  const week = plan?.weeks.find((w) => w.weekNumber === selectedWeek);
  const day = week?.days?.find((d) => d.dayNumber === selectedDay);
  const weekUnlocked = week && isWeekUnlocked(week.weekNumber, garbhaUnlockedWeek);
  const dayUnlocked = plan?.isDayWise
    ? day && isDayUnlocked(day.dayNumber, selectedWeek, garbhaUnlockedWeek, garbhaUnlockedDay)
    : true;
        ...
        <>
          <Card className="mt-6">
            ...
            <p className="mt-3 text-sm">
              Unlock: <Badge color="green">Week {garbhaUnlockedWeek}</Badge>
              {plan.isDayWise && (
                <span className="ml-2">
                  <Badge color="green">Day {garbhaUnlockedDay}</Badge>
                </span>
              )}
            </p>
          </Card>
          ...
          <div className="mt-4">
            <PatientWeekPicker
              ...
              unlockedWeek={garbhaUnlockedWeek}
